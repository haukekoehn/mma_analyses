...
def inclination_function(key, params, luminosity_distance):
    ...
    u = jax.random.uniform(subkey1, shape = luminosity_distance.shape, minval=0., maxval=1.)
    u = jnp.where(
        luminosity_distance > params["beta_1"],
        jnp.sqrt(u*(limit-1)**2) + limit,
        u,
    )


    u = u * jax.random.choice(subkey2, jnp.array([-1, 1]), shape=u.shape)
    return jnp.arccos(u)
...
@jax.jit
def gw_detection_probability(m1: np.ndarray, m2: np.ndarray, luminosity_distance: np.ndarray, theta_jn: np.ndarray):
...
@jax.jit
def em_detection_probability(m1: np.ndarray, m2: np.ndarray, luminosity_distance: np.ndarray, inclination_EM: np.ndarray, sample: dict, gw_detected: np.ndarray):
...
def detection_prob_per_sample(sample: dict):
    ...
    theta_jn = inclination_function(jax.random.key(38992), sample, luminosity_distance)
    inclination_EM = jnp.arccos(jnp.abs(jnp.cos(theta_jn)))


    gw_prob, gw_detected = gw_detection_probability(m1, m2, luminosity_distance, theta_jn)
