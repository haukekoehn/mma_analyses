...
def check_truth():
    ...
    truth.update(dict(H0=67.66, Omega0=0.30966))
    truth = transform.forward(truth)

    for j in range(46,events.shape[0]):
        logl_bestposterior= likelihoods[j+2].evaluate(best_posterior)
        logl_truth = likelihoods[j+2].evaluate(truth)

        print(j, events.loc[j, "redshift"], np.cos(events.loc[j, "theta_jn"]), logl_bestposterior, logl_truth, logl_truth - logl_bestposterior)
