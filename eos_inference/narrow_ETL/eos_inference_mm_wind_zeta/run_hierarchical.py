...
for src in range(events.shape[0]):
    ...
    zeta = 10**(events.loc[src, "log10_mej_wind"] - events.loc[src, "log10_mdisk"])
    ...
    likelihood = MultiMessengerLikelihood(
        ...
        flow_em = f"../../../cosmo_inference/narrow_ETL/em_posteriors/source_{src}/nf",
        cflow_em = f"../../../cosmo_inference/narrow_ETL/em_posteriors/source_{src}/cnf",
        wind_zeta = zeta,
        use_em=True,
