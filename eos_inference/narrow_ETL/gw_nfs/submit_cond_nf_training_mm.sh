...
#SBATCH -J narrow_ETL_cnf_mm_${src}
#SBATCH -o ./source_${src}/cnf_mm/log_cnf
#SBATCH -e ./source_${src}/cnf_mm/log_cnf
...
cat <<CONFIG_EOF > cnf_config_mm_${src}.yaml

# Configuration for training normalizing flow
# Updated to use new default hyperparameters (PR #55)
posterior_file: ../gw_posteriors/source_${src}/posterior_mm.npz
output_dir: ./source_${src}/cnf_mm
...
cond_dim: 4
cond_parameter_names: ["mass_1_source", "mass_2_source", "luminosity_distance", "cos_theta_jn"]
CONFIG_EOF

train_jester_flow "./cnf_config_mm_${src}.yaml"
rm ./cnf_config_mm_${src}.yaml
SBATCH_EOF
